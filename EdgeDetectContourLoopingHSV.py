...
img = cv2.imread('Images\\test19\.jpg') 
img = cv2.resize(img,(512,512))
img = cv2.cvtColor(img, cv2.COLOR_RGB2HSV)
original = img
# cv2.blur(img, (15,15)) 
# Display original image
cv2.imshow('Original', cv2.cvtColor(original,cv2.COLOR_HSV2RGB))
...
cv2.createTrackbar('H','image',0,255,lambda x:None)
cv2.createTrackbar('S','image',0,255,lambda x:None)
cv2.createTrackbar('V','image',0,255,lambda x:None)
cv2.createTrackbar('H2','image',0,255,lambda x:None)
cv2.createTrackbar('S2','image',0,255,lambda x:None)
cv2.createTrackbar('V2','image',0,255,lambda x:None)
cv2.createTrackbar('minPixels','image',0,10000,lambda x:None)
...
while(True):
    ...
    yellowMask = cv2.inRange(yellowMask,(110,100,115),(255,255,255))

    # yellowMask = cv2.inRange(yellowMask, (h,s,v),(h2,s2,v2))
    ...
    cv2.drawContours(image=image_copy, contours=new_contours, contourIdx=-1, color=(140, 255, 140), thickness=2, lineType=cv2.LINE_AA)
        
    cv2.imshow('Contours', cv2.cvtColor(image_copy,cv2.COLOR_HSV2RGB))#image_copy)
                
    img = cv2.bitwise_and(original, original, mask=yellowMask)
    cv2.imshow('Masked',cv2.cvtColor(img, cv2.COLOR_HSV2RGB))
